...
function TrangThaiDeTai() {
    ...
    const handleUpdate = (dc) => {
        if (dc) {
            ...
            const fetchAPI = async () => {
                const res = await request.put(`/detai/${detai.madt}`, { decuong: dc }, {
                    Headers: { 'Content-Type': 'application/json; charset=utf-8' }
                });
                inputFileRef.current.value = "";
                res && alert("Nộp đề cương thành công");
            };
            fetchAPI();
    ...
    useEffect(() => {
        const fetchAPI = async (masv) => {
            ...
            if (ressv[0]) {
                const resdt = await request.get("/detai", { params: { manhom: ressv[0].manhom } });
                const resdstv = await request.get('/nhom', { params: { manhom: ressv[0].manhom, vaitro: "Thành viên" } });
                const reskq = await request.get('/ketqua', { params: { manhom: ressv[0].manhom } });
                reskq[0] && setKetqua(reskq[0]);
                setDetai(resdt[0]);
                setDsthanhvien(resdstv);
                setDecuong(resdt[0]?.decuong);
            }
            setSinhvien(ressv[0]);
            setIsLoading(false);
        };
